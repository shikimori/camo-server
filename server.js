...
(function() {
  var Crypto, Fs, Http, Https, Path, QueryString, Url, accepted_image_mime_types, allowed_hosts, camo_hostname, content_length_limit, current_connections, debug_log, default_security_headers, default_transferred_headers, endpoint_path, endpoint_path_regex, error_log, finish, four_oh_four, hexdec, keep_alive, logging_enabled, max_redirects, port, process_url, server, shared_key, socket_timeout, started_at, total_connections, version,
  ...
  if (endpoint_path) {
    endpoint_path_regex = new RegExp("^" + endpoint_path);
  }
  ...
  default_transferred_headers = ['if-modified-since', 'if-none-match'];
        ...
        if (content_length > content_length_limit) {
          ...
          switch (srcResp.statusCode) {
            ...
            case 304:
              ...
              return finish(resp, "Not Modified");
            default:
  ...
  server = Http.createServer(function(req, resp) {
    var base, dest_url, encoded_url, error, filename, hmac, hmac_digest, pathname, query_digest, query_params, ref, ref1, ref2, transferredHeaders, url, url_type, user_agent;
    if (req.method !== 'GET' || req.url === '/') {
      ...
      for (j = 0, len = default_transferred_headers.length; j < len; j++) {
        header = default_transferred_headers[j];
        if (req.headers[header]) {
          transferredHeaders[header] = req.headers[header];
        }
      }
      delete req.headers.cookie;
      pathname = endpoint_path_regex ? url.pathname.replace(endpoint_path_regex, '') : url.pathname;
      ref2 = pathname.replace(/^\//, '').split("/", 2), query_digest = ref2[0], encoded_url = ref2[1];
